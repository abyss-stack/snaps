...
C = NS(
    gradient=mcolors.LinearSegmentedColormap.from_list(
        "pie", ["#2d353b", "#4b564c", "#5c6a5e", "#7a8478", "#8da189", "#a7b09e"]
    ),
    main="#434f46",
    txt="#7a8478",
    edge="#4b564c",
...
def main():
    ...
    out = sys.argv[1] if len(sys.argv) > 1 else "codebase_size.png"
    ...
    total_loc  = sum(locs)
    n = len(sizes)
    colors = [C.gradient(i / max(n - 1, 1)) for i in range(n)]
    ...
    ax1.pie(
        sizes,
        wedgeprops=dict(width=0.3, edgecolor=C.edge, lw=0.8),
        startangle=100,
        colors=colors,
    )
    ax1.text(
    ...
    y, step = 0.85, 0.055
    ax2.text(0.05, y + 0.055, "FILE PATH",       fontfamily="monospace", fontsize=10, color=C.main, weight="bold")
    ax2.text(0.55, y + 0.055, " SIZE | METRICS", fontfamily="monospace", fontsize=10, color=C.main, weight="bold")
    ax2.plot([0.05, 0.95], [y + 0.035, y + 0.035], color=C.txt, lw=1, alpha=0.6)

    for name, size, loc, color in zip(labels, sizes, locs, colors):
        short = name if len(name) <= 35 else name[:32] + "..."
        ax2.text(0.05, y, short, fontfamily="monospace", fontsize=11, color=color, va="center", weight="bold")
        ax2.text(0.55, y, f"{size / 1024:>7.1f} KB | {loc:>6} LOC",
