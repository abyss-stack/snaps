...
pub struct Recipe {
    ...
    pub non_btrfs_entries: Vec<NonBtrfsEntry>,
}
...
impl Recipe {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> AppResult<Self> {
        ...
        let data = std::fs::read_to_string(path)
            .map_err(|e| AppError::RecipeLoadError {
                path: path.to_path_buf(),
                what: e.to_string(),
            })?;

        let mut recipe: Self = serde_json::from_str(&data)
            .map_err(|e| AppError::RecipeParseError {
                what: e.to_string(),
            })?;

        if let Some(layout) = &mut recipe.btrfs_layout {
            layout.init_tracked_set();
            AppMessage::TrackedSetInited {
                count: layout.tracked_set.len(),
            }.emit();
        }
